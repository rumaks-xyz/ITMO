...
Функция $f$ называется дифференцируемой в точке $x_0$, если существует конечная производная $f^\prime(x_0)$. Дифференциалом $f$ в точке $x_0$ называется линейное отображение $df(x_0): h \mapsto f\prime(x_0) h$, то есть

$$df(x_0) = f^\prime(x_0) h$$

### 9. Правосторонняя производная
Правосторонняя производная в точке $x_0$ — это предел

$$f^\prime_+(x_0) = \lim_{h \to 0^+}\frac{f(x_0+h)-f(x_0)}{h}$$
...
Вертикальная касательная в точке $x_0$ возникает, если $\lim_{x \to x_0} f^\prime(x) = \pm \infty$. Тогда уравнение касательной — вертикальная прямая $x = x_0$.
...
Гладкий экстремум — это локальный экстремум в точке, где функция дифференцируема и $f^\prime(x_0)=0$, при этом сама функция достаточно гладка (нет излома).
...
### 22. Точка перегиба
Точка $x_0$ — точка перегиба, если в ней функция меняет характер кривизны: при переходе через $x_0$ $f$ меняется с выпуклой на вогнутую или наоборот. При наличии второй производной обычно требуется, что $f^\prime\prime(x_0)=0$ и знаки $f\prime\prime(x)$ слева и справа от $x_0$ различны.
...
Функция $f$ дифференцируема в точке $x_0$ тогда и только тогда, когда существует конечная производная $f^\prime(x_0)$.
...
- $\left(\frac{f}{g}\right)^\prime(x) = \frac{f\prime(x)g(x)-f(x)g\prime(x)}{[g(x)]^2}$ при $g(x)\neq 0$.
...
Аналогично для дифференциалов: $dy = f^\prime(g(x))\,dg(x)$.
...
### 18. Теорема Ферма
Если $f$ имеет локальный экстремум в точке $x_0$ и дифференцируема в $x_0$, то $f^\prime(x_0)=0$.

### 19. Теорема Ролля
Если $f$ непрерывна на $[a,b]$, дифференцируема на $(a,b)$ и $f(a)=f(b)$, то существует $c \in (a,b)$ такой, что $f^\prime(c)=0$.
...
### 21. Теорема Коши
Если $f$ и $g$ непрерывны на $[a,b]$, дифференцируемы на $(a,b)$, и $g^\prime(x)\neq0$ на $(a,b)$, то существует $c\in(a,b)$, для которого

$$\frac{f^\prime(c)}{g\prime(c)} = \frac{f(b)-f(a)}{g(b)-g(a)}$$

### 22. Правило Лопиталя
Пусть при $x \to a$ функции $f(x)$ и $g(x)$ стремятся к $0$ или обе — к $\pm\infty$, и $f,g$ дифференцируемы в окрестности $a$, при этом $g^\prime(x)\neq 0$ для $x \neq a$. Если существует предел
...
Если $f^\prime(x)>0$ для всех $x$ в интервале, то $f$ строго возрастает на этом интервале; если $f\prime(x)<0$ — строго убывает.

### 24. Критерий постоянства функции
Если $f^\prime(x)=0$ для всех $x$ в интервале, то $f$ постоянна на этом интервале.
...
Знак производной определяет направление изменения функции: $f^\prime(x)>0$ — рост, $f\prime(x)<0$ — убывание.

### 28. Необходимое условие экстремума
Если в точке $x_0$ внутреннего экстремума функция дифференцируема, то $f^\prime(x_0)=0$.

### 29. Первое достаточное условие экстремума
Пусть $f^\prime(x_0)=0$. Если при переходе через $x_0$ знак $f\prime(x)$ меняется с «+» на «-», то $x_0$ — точка локального максимума; если с «-» на «+», то локального минимума.

### 30. Второе достаточное условие экстремума
Если $f^\prime(x_0)=0$ и $f\prime\prime(x_0)<0$, то $x_0$ — локальный максимум; если $f\prime\prime(x_0)>0$ — локальный минимум.

### 31. Критерий выпуклости дважды дифференцируемой функции
Если $f^\prime\prime(x)>0$ на интервале, $f$ выпукла там; если $f\prime\prime(x)<0$, то вогнута.
